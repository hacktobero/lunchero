...
export default function Tab({ day, onDayChange, active, role }) {
    return (
        <button role={role} onClick={() => { onDayChange(day) }}
                className={`md:w-1/5 md:h-full sm:w-3/5 sm:h-1/5 sm:my-1 md:rounded-t-2xl md:rounded-b-sm sm:rounded-2xl flex duration-200 text-white font-bold justify-center shadow-sm self-center items-center
